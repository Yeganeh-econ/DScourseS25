...
income <- income %>% select(-native.country, -fnlwgt, -education.num)
# Make sure continuous variables are formatted as numeric
income <- income %>% mutate(across(c(age, hours, capital.gain, capital.loss), as.numeric))
# Make sure discrete variables are formatted as factors
income <- income %>% mutate(across(c(high.earner, education, marital.status, race, workclass, occupation, relationship, sex), as.factor))
# Combine levels of factor variables that currently have too many levels
income <- income %>% mutate(education = fct_collapse(education,
...
print(dim(income_train))
print("Dimensions of test set:")
print(dim(income_test))
...
library(dials)      # For grid_regular()
library(glmnet)     # For logistic regression with lasso/ridge
...
library(workflows)   # For add_formula() and workflow()
...
library(tune)        # For tune() and show_best()
...
# lambda is the tuning parameter for regularization
...
print('Starting NNET')
# set up the task and the engine
# MLP stands for Multi-Layer Perceptron, which is a type of feedforward artificial neural network and it creates a Multi-Layer Perceptron neural network model specification.
tune_nnet_spec <- mlp(
...
# seq is from 1 to 10 for hidden units because neural networks can have multiple hidden layers
nnet_parm_df1 <- tibble(hidden_units = seq(1,10))
nnet_lambda_grid <- grid_regular(penalty(), levels = 10)
nnet_parm_df  <- full_join(nnet_parm_df1, nnet_lambda_grid, by=character())
...
#####################
# SVM - As It was mentioned in class: MODIFIED to 3x3 grid
#####################
print('Starting SVM')
# set up the task and the engine
...
# MODIFIED: I Reduced to 3x3 grid instead of 6x6 in order to get the result faster!
# Using only 3 key values instead of 6
svm_parm_df1 <- tibble(cost      = c(2^(-2), 2^0, 2^2))
svm_parm_df2 <- tibble(rbf_sigma = c(2^(-2), 2^0, 2^2))
svm_parm_df  <- full_join(svm_parm_df1, svm_parm_df2, by=character())

# Print the dimensions of the parameter grid
print("Dimensions of SVM parameter grid:")
print(dim(svm_parm_df))
